//! OAuth2 token management for Google Cloud APIs.
//!
...
use crate::error::GcpAuthError;
use crate::types::{AuthorizedUserCredentials, GOOGLE_TOKEN_URL, ServiceAccountKey, TokenResponse};

/// Manages OAuth2 tokens for Google Cloud API access.
///
/// Caches tokens and refreshes them before expiry. Thread-safe via `RwLock`.
/// The `scope` field determines the OAuth2 scope requested (e.g., GCS read-only
/// or Drive read-only).
#[derive(Debug)]
...
impl TokenProvider {
    /// Create a new token provider for the given credential reference and scope.
    pub fn new(credential_ref: CredentialRef, http_client: reqwest::Client, scope: &str) -> Self {
        Self {
            credential_ref,
            http_client,
    ...
    pub async fn get_token(&self) -> Result<String, GcpAuthError> {
        {
        ...
        debug!("refreshing Google Cloud access token");
    ...
    async fn refresh_token(&self) -> Result<CachedToken, GcpAuthError> {
        match &self.credential_ref {
            ...
            CredentialRef::Secret { name } => Err(GcpAuthError::InvalidCredentials {
    ...
    async fn service_account_flow(&self, path: &PathBuf) -> Result<CachedToken, GcpAuthError> {
        let key_json = tokio::fs::read_to_string(path).await.map_err(|e| {
            GcpAuthError::InvalidCredentials {
                message: format!(
        ...
        let key: ServiceAccountKey =
            serde_json::from_str(&key_json).map_err(|e| GcpAuthError::InvalidCredentials {
    ...
    fn create_service_account_jwt(
        ...
        let encoding_key = jsonwebtoken::EncodingKey::from_rsa_pem(key.private_key.as_bytes())
            .map_err(|e| GcpAuthError::Auth {
                message: format!("invalid RSA private key: {e}"),
            })?;

        let header = jsonwebtoken::Header::new(jsonwebtoken::Algorithm::RS256);
        jsonwebtoken::encode(&header, &claims, &encoding_key).map_err(|e| GcpAuthError::Auth {
    ...
    async fn exchange_jwt_for_token(
        ...
        jwt: &str,
        token_url: &str,
    ) -> Result<CachedToken, GcpAuthError> {
        let response = self
            .http_client
            .post(token_url)
            .form(&[
    ...
    fn env_var_flow(env_var: &str) -> Result<CachedToken, GcpAuthError> {
        let token = std::env::var(env_var).map_err(|_| GcpAuthError::Auth {
    ...
    async fn adc_flow(&self) -> Result<CachedToken, GcpAuthError> {
    ...
    pub(crate) async fn resolve_credential_file(
        ...
        Err(GcpAuthError::InvalidCredentials {
...
pub fn well_known_adc_path() -> Option<PathBuf> {
...
mod tests {
    ...
    fn test_static_token_returns_immediately() {
        let provider = TokenProvider::static_token("test-token".to_string());
    ...
    fn test_env_var_flow_missing_var() {
        let result = TokenProvider::env_var_flow("ECL_GCP_AUTH_TEST_NONEXISTENT_VAR_12345");
    ...
    fn test_with_scope() {
        ...
        .with_scope("scope-2".to_string());
        assert_eq!(provider.scope, "scope-2");
    }
}
